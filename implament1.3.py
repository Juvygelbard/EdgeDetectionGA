...
class DissimilarityRegion():
    ...
    def __convert_to_pos__(self, struct):
        # find c pos
        c_pos = zip(*np.where(struct>=20)).__next__()
        # build poses
        r1 = [(x-c_pos[1], y-c_pos[0]) for (y, x) in zip(*np.where(struct%10==1))]
        r2 = [(x-c_pos[1], y-c_pos[0]) for (y, x) in zip(*np.where(struct%10==2))]
        Xs = [(x-c_pos[1], y-c_pos[0]) for (y, x) in zip(*np.where(struct>=10))]
...
factors = [(0.5, curvature_factor),
           ...
           (4, fragmentation_factor),
           (8.51, thickness_factor)]
